...
public class EveningOperator : Operator
{
    ...
    [SerializeField] private Vector3 st1Pos = new Vector3(-330, 220, 0),
        st2Pos = new Vector3(-75, 220, 0),
        elstPos = new Vector3(200, 220, 0);

    [SerializeField] private Vector3 base1Pos = new Vector3(-330, 220, 0),
        base2Pos = new Vector3(-75, 220, 0),
        elementPos = new Vector3(200, 220, 0);
    ...
    {
        StartCoroutine(RerollBase(true));
        StartCoroutine(RerollElement(true));
    }

    public void RerollB(bool b) => StartCoroutine(RerollBase(b));

    IEnumerator RerollBase(bool first)
    {
        if (twistsBase > 0 && (storage.PepperCount >= priceBase || first))
        {
            ...
            if (!first)
            {
                ...
                priceBase *= 2;
                upgraderBase1.transform.DOLocalMoveY(-700, 1f);
                upgraderBase2.transform.DOLocalMoveY(-700, 1f);
            ...
            upgraderBase1.transform.DOLocalMove(base1Pos, 1f);
            upgraderBase2.transform.DOLocalMove(base2Pos, 1f);
    ...
    public void RerollEl(bool el) => StartCoroutine(RerollElement(el));

    IEnumerator RerollElement(bool first)
    {
        if (twistsElement > 0 && (storage.PepperCount >= priceElement || first))
        {
            ...
            if (!first)
            {
                ...
                priceElement *= 2;
                upgraderElement.transform.DOLocalMoveY(-700, 1f);
                yield return new WaitForSeconds(1);
                Destroy(upgraderElement);
            }
            ...
            upgraderElement.transform.DOLocalMove(elementPos, 1f);
